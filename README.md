## Final Project Submission Checklist

**Name:** Youssef Gaballa   **MyID:** yg01467

### eLC Submission
- [x] All code files uploaded in code/ folder
- [x] Code runs without errors
- [] README.txt explains how to run your project

### Video Demo (3-5 min)
- [ ] Unlisted video link provided in report/video_link.txt
- [ ] Has voiceover explaining your work
- [ ] Demonstrates key features of your project

### Report (1,200 words) in report/report.pdf
- [x] Project description and graphics techniques used
- [x] Steps taken and development process
- [x] Challenges encountered and solutions
- [ ] Results with screenshots
- [ ] Effort Justification: How you met the 1.5x effort level
      - What makes this more substantial than a typical assignment?
      - What new techniques did you learn or extend?
      - If team: how does EACH member contribute 1.5x effort?
- [ ] Rubric Checklist: How you fulfilled your proposed rubric
      (If team: each member submits their own checklist)

### Rubric (in rubric/ folder)
- [ ] Your proposed rubric (from proposal) is included
- [ ] Self-evaluation showing points earned per criterion

### Piazza / eLC Post
- [ ] Posted showcasing your final project
- [ ] Includes brief description and key results/screenshots

### Presentation
- [ ] Prepared slides for 3-5 minute presentation
- [ ] Will attend full exam session (Tue May 5, 3:30-6:30 PM)